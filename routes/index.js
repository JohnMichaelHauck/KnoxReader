...
router.get('/', function (req, res, next) {
  fs.readFile("history_reformation.txt", 'utf8', function (err, fileText) { res.send(convertBook(fileText)) });
  //res.send("Howdy James".replace("James", "Fred"));
});
...
function convertBook(fileText) {
  ...
  // classes = 'place';
  // encode(/§/, /Market Cross of Edinburgh/g, classes);
  ...
  // encode(/§/, /Haddington/g, classes);
  // encode(/§/, /Falkland/g, classes);
  // encode(/§/, /Castle of Carny/g, classes);
